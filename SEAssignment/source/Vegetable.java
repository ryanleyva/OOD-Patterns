...
public class Vegetable implements GardenEntity {
	Point2D point;
	Color color;
	Circle circle;
	GardenEntity parent;
	public Vegetable(Point2D point, Color color)
	{
		this.point = point;
		this.color = color;
		genFlower();
	...
	public GardenEntitySerializable serialize() {
		return new VegetableSerializable(point.getX(), point.getY(), circle.getRadius(), color.toString());
	}
}
